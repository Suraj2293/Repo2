...
public class FinalInsertDaoImpl extends DaoUtils implements Serializable {
	...
	public void insertData(MultiEventCoreDTO multiEventCoreDTO,
			Connection connection, PreparedStatement psParentChild,
			PreparedStatement psRuleMatch) {
		try {
			...
			String dt = Arrays.toString(multiEventCoreDTO.getDataArray());
			dt = dt.substring(1, dt.length() - 1);
			psRuleMatch.setString(7, dt);
			psRuleMatch.executeUpdate();
		} catch (Exception e) {
			log.error("Exception in FinalInsertDaoImpl, method: insertData, Error is: "
					+ e.getMessage());
		}

		if (multiEventCoreDTO.getRuleMatchDTO().getChildIdList() != null
				&& !multiEventCoreDTO.getRuleMatchDTO().getChildIdList()
						.isEmpty()) {
			try {
				LongOrNull(1, psParentChild, multiEventCoreDTO.getThreatId());
				IntOrNull(2, psParentChild, multiEventCoreDTO.getRuleMatchDTO()
						.getRuleId());
				psParentChild.setArray(
						3,
						connection.createArrayOf("BIGINT", multiEventCoreDTO
								.getRuleMatchDTO().getChildIdList().toArray()));
				TimestampOrNull(4, psParentChild,
						multiEventCoreDTO.getEventTime());
				psParentChild.setString(5,
						clientProp.getProperty("threatRaw.tableName"));
				psParentChild.setString(6, multiEventCoreDTO.getRuleMatchDTO()
						.getTimeWindowValue()
